...
class ReviewController extends GetxController implements GetxService {
  ...
  Future<void> getReviewList(int offset, {bool reload = true}) async {
    ...
    Response response = await reviewServiceInterface.getReviewList(
      offset,
      _selectedReviewType == 'regular' ? 0 : 1,
    );
    if (response.statusCode == 200) {
      if (offset == 1) {
        ...
        _reviewModel!.review!.addAll(
          ReviewModel.fromJson(response.body).review!,
        );
      }
    } else {
  ...
  Future<void> savedReview(int? reviewId, int isSaved, int? index) async {
    ResponseModel responseModel = await reviewServiceInterface.saveReview(
      reviewId,
      isSaved,
    );
