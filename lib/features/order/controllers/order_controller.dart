...
class OrderController extends GetxController implements GetxService {
  ...
  Future<void> getAllOrderHistory(
    ...
    _isLoading = true;
    Response response = await orderServiceInterface.getAllOrderHistory(
      dateType,
      type,
      startDate,
      endDate,
    ...
    if (response.body != null && response.statusCode == 200) {
      ...
      if (type == 'delivered') {
        ...
        response.body.forEach((order) {
          deliveredOrderHistory!.add(OrderModel.fromJson(order));
        });
      } else if (isPause == 1) {
        pauseOrderHistory = [];
        response.body.forEach((order) {
          pauseOrderHistory!.add(OrderModel.fromJson(order));
        });
      } else {
        _allOrderHistory = [];
        response.body.forEach((order) {
