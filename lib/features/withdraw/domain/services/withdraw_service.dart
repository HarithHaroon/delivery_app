...
import 'package:rawabit_delivery_app/features/withdraw/domain/models/withdraw_model.dart';
...
class WithdrawService implements WithdrawServiceInterface {
  ...
  Future<Response> sendWithdrawRequest({String? amount, String? note}) async {
    Response response = await withdrawRepoInterface.sendWithdrawRequest(
      amount: amount,
      note: note,
    );
  ...
  @override
  Future getWithdrawList({
    String? startDate,
    String? endDate,
    int? offset,
    String? type,
  }) async {
    Response response = await withdrawRepoInterface.getWithdrawList(
      startDate: startDate,
      endDate: endDate,
      offset: offset,
      type: type,
