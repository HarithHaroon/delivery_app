...
class OrderDetailsController extends GetxController implements GetxService {
  ...
  Future<List<OrderDetailsModel>?> getOrderDetails(
    ...
    _orderDetails = null;
    Response response = await orderDetailsServiceInterface.getOrderDetails(
      orderID: orderID,
    );
  ...
  Future<bool> updateOrderStatus({
    ...
    _isLoading = true;
    update();
    Response response = await orderDetailsServiceInterface.updateOrderStatus(
      orderId: orderId,
      status: status,
    );
    bool _isSuccess;
  ...
  Future<bool> cancelOrderStatus({
    ...
    _isLoading = true;
    update();
    bool _isSuccess = await orderDetailsServiceInterface.cancelOrderStatus(
      orderId: orderId,
      cause: cause,
    );
    Get.back();
  ...
  Future<bool> rescheduleOrderStatus({
    ...
    _isLoading = true;
    update();
    bool _isSuccess = await orderDetailsServiceInterface.rescheduleOrder(
      orderId: orderId,
    ...
    if (_isSuccess) {
      showCustomSnackBarWidget(
        'order_status_rescheduled_successfully'.tr,
        isError: false,
      );
    }
    _isLoading = false;
  ...
  Future<bool> pauseAndResumeOrder({
    ...
    _isLoading = true;
    update();
    bool _isSuccess = await orderDetailsServiceInterface.pauseAndResumeOrder(
      orderId: orderId,
      isPos: isPos,
      cause: cause,
    );
  ...
  Future<Response?> updatePaymentStatus({int? orderId, String? status}) async {
    Response apiResponse = await orderDetailsServiceInterface
  ...
  void setEarningFilterIndex(int index) {
    ...
    if (_orderTypeFilterIndex == 0) {
      ...
      Get.find<WalletController>().getOrderWiseDeliveryCharge(
        '',
        '',
        1,
        'TodayEarn',
  ...
  void pickImage({bool camera = false}) async {
    ...
    if (camera) {
      ...
      identityImage =
          (await ImageValidationHelper.validateAndPickMultipleImages(
    ...
    if (identityImage != null) {
      for (XFile image in identityImage!) {
        ...
        if (value >
          ...
          showCustomSnackBarWidget(
            '${'maximum_image_size'.tr} ${(configModel?.systemImageFileUploadMaxSize ?? AppConstants.fileImageMaxLimit)}MB',
          );
          return;
        } else {
  ...
  Future<Response> uploadOrderVerificationImage(String oderId) async {
    uploading = true;
    update();
    Response? response = await orderDetailsServiceInterface
        .uploadOrderVerificationImage(oderId, multipartList);
  ...
  Future<Response?> otpVerificationForOrderVerification({
    ...
    _isLoading = true;
    Response apiResponse = await orderDetailsServiceInterface
        .verifyOrderDeliveryOtp(orderId: orderId, verificationCode: otp);
  ...
  Future resendOtpForOrderVerification({int? orderId}) async {
    await orderDetailsServiceInterface.resendOtpForOrderVerification(
      orderId: orderId,
    );
    update();
  }
