...
import 'package:rawabit_delivery_app/features/profile/widgets/account_info_widget.dart';
import 'package:rawabit_delivery_app/features/profile/widgets/general_info_widget.dart';
import 'package:rawabit_delivery_app/features/profile/widgets/profile_header.dart';
...
class _ProfileEditScreenState extends State<ProfileEditScreen>
  ...
  void _updateUserAccount() async {
    if (Get.find<ProfileController>().profileModel!.fName ==
            Get.find<ProfileController>().firstNameController.text &&
        Get.find<ProfileController>().profileModel!.lName ==
            Get.find<ProfileController>().lastNameController.text &&
        Get.find<ProfileController>().profileModel!.address ==
            Get.find<ProfileController>().addressController.text &&
      ...
      showCustomSnackBarWidget('change_something_to_update'.tr);
            ...
            .text
            .isNotEmpty &&
        !Get.find<ProfileController>().isPasswordValid()) {
      ...
      showCustomSnackBarWidget('password_be_at_least_8_character'.tr);
            ...
            .text
            .isNotEmpty &&
        Get.find<ProfileController>().passwordController.text !=
            Get.find<ProfileController>().confirmPasswordController.text) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: GetBuilder<ProfileController>(
        builder: (profile) {
          int idImageSize =
              profile.profileModel?.identityImageFullUrl?.length ?? 0;
          if (profile.firstNameController.text.isEmpty ||
              profile.lastNameController.text.isEmpty) {
