...
class ChatRepository implements ChatRepositoryInterface {
  ...
  Future<Response> getConversationList(int offset, String userType) async {
    return apiClient.getData(
      '${AppConstants.chatListUri}$userType?limit=10&offset=$offset',
    );
  }
  ...
  Future<Response> getChatList(int offset, String userType, int? id) async {
    return await apiClient.getData(
      '${AppConstants.messageListUri}$userType/$id?offset=$offset&limit=25',
    );
  }
  ...
  Future<Response> searchChatList(String userType, String search) async {
    return await apiClient.getData(
      '${AppConstants.chatSearch}$userType?search=$search',
    );
  }
  ...
  Future<Response> sendMessage(
    ...
    return apiClient.postMultipartData(
      ...
      {'message': message, 'id': userId.toString()},
      _getMultipartBody(files),
      platformFile: (platformFile != null && platformFile.isNotEmpty)
          ? platformFile
          : null,
    );
  }
