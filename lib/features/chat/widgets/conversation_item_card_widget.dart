...
import 'package:rawabit_delivery_app/common/basewidgets/custom_ink_well_widget.dart';
...
import 'package:rawabit_delivery_app/helper/shop_helper.dart';
import 'package:rawabit_delivery_app/helper/string_extensions.dart';
import 'package:rawabit_delivery_app/features/chat/screens/chat_screen.dart';
  ...
  Widget build(BuildContext context) {
    return GetBuilder<ChatController>(
      builder: (chatController) {
        final inHouseShop =
            Get.find<SplashController>().configModel?.inHouseShop;
        ...
        String name = chatController.userTypeIndex == 0
            ? chat!.sellerInfo != null
        ...
        int? userId = chatController.userTypeIndex == 0
            ? chat?.sellerId
            : chatController.userTypeIndex == 1
            ? chat?.userId
            : inHouseShop?.sellerId;
        ...
        return Container(
          ...
          decoration: BoxDecoration(
            ...
            color: (chat?.seenByDeliveryMan ?? false)
                ? Get.isDarkMode
          ...
          child: CustomInkWellWidget(
            onTap: () => Get.to(
              ChatScreen(
                ...
                name: name,
                image: image,
                isShopOnVacation: chatController.userTypeIndex == 0
                    ? ShopHelper.isVacationActive(
                        context,
                        startDate: DateTime.tryParse(
                          chat?.sellerInfo?.shops?[0].vacationStartDate ?? '',
                        ),
                        endDate: DateTime.tryParse(
                          chat!.sellerInfo!.shops?[0].vacationEndDate ?? '',
                        ),
                        vacationDurationType:
                            chat!.sellerInfo!.shops?[0].vacationDurationType,
                        vacationStatus:
                            chat!.sellerInfo!.shops?[0].vacationStatus,
                    ...
                    children: [
                      Stack(
                        children: [
                          ...
                          if (chat?.sellerInfo?.shops?[0].temporaryClose ??
                              false)
                            Container(
                              height: 50,
                              width: 50,
                                ...
                                Expanded(
                                  child: Text(
                                    ...
                                    style: rubikBold.copyWith(
                                      color:
                                          chatController.userTypeIndex == 0 &&
                                              chat!.sellerInfo == null
                                          ? Colors.red
                                          : chatController.userTypeIndex == 1 &&
                                                chat!.customer == null
