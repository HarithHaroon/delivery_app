...
class MessageBubbleWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    String? image = _getAvatarImage(
      userTypeIndex: Get.find<ChatController>().userTypeIndex,
    );
    ...
    return GetBuilder<ChatController>(
      builder: (chatController) {
        ...
        bool isSameUserWithPreviousMessage = chatController
            .isSameUserWithPreviousMessage(previous, message);
        bool isSameUserWithNextMessage = chatController
            .isSameUserWithNextMessage(message, next);
  ...
  String? _getAvatarImage({required int userTypeIndex}) {
    return userTypeIndex == 0
        ? message.sellerInfo != null
              ...
              : ''
        : userTypeIndex == 1
        ? message.customer?.imageFullUrl?.path ?? ''
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: AnimatedContainer(
        ...
        height: chatController.onMessageTimeShowID == message.id.toString()
            ? 25.0
            : 0.0,
        child: Padding(
          padding: EdgeInsets.only(
        ...
        children: [
          if (message.message != null && message.message!.isNotEmpty)
            InkWell(
              ...
              child: Container(
                margin: isMe
                    ? const EdgeInsets.fromLTRB(70, 4, 10, 2)
                    : const EdgeInsets.fromLTRB(10, 4, 40, 2),
    ...
    for (var image in widget.images) {
      if (chatController.isVideoExtension(image.path ?? '')) {
        if (_thumbnailCache.containsKey(image.path)) {
          ...
          final thumbnail = await chatController.generateThumbnail(
            image.path ?? '',
          );
