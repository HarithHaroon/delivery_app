...
import 'package:rawabit_delivery_app/features/auth/domain/models/response_model.dart';
import 'package:rawabit_delivery_app/features/chat/domain/models/chat_model.dart';
import 'package:rawabit_delivery_app/features/chat/domain/models/media_file_model.dart';
import 'package:rawabit_delivery_app/features/chat/domain/models/message_model.dart';
...
import 'package:rawabit_delivery_app/features/splash/domain/models/config_model.dart'
    hide Colors;
import 'package:rawabit_delivery_app/helper/date_converter.dart';
import 'package:rawabit_delivery_app/helper/image_size_checker.dart';
...
class ChatController extends GetxController implements GetxService {
  ...
  Future<void> getConversationList(int offset, {bool isUpdate = true}) async {
    ...
    _isLoading = true;
    Response response = await chatServiceInterFace.getConversationList(
      offset,
      _userTypeIndex == 0
          ? 'seller'
          : _userTypeIndex == 1
    ...
    if (response.statusCode == 200) {
      if (offset == 1) {
        ...
        _conversationModel!.totalSize = ChatModel.fromJson(
          response.body,
        ).totalSize;
        _conversationModel!.offset = ChatModel.fromJson(response.body).offset;
        _conversationModel!.chat!.addAll(
          ChatModel.fromJson(response.body).chat!,
        );
      }
    } else {
  ...
  Future<void> searchConversationList(String searchChat) async {
    ...
    _isLoading = true;
    _conversationModel = await chatServiceInterFace.searchChatList(
      _userTypeIndex == 0
          ? 'seller'
          : _userTypeIndex == 1
          ? 'customer'
  ...
  Future<void> getChats(
    ...
    if (_response.body != {} && _response.statusCode == 200) {
      if (offset == 1) {
        ...
        _messageModel?.totalSize = MessageModel.fromJson(
          _response.body,
        ).totalSize;
        _messageModel?.offset = MessageModel.fromJson(_response.body).offset;
        _messageModel?.message?.addAll(
          MessageModel.fromJson(_response.body).message ?? [],
        );
      }
    } else {
  ...
  Future<ResponseModel> sendMessage(String message, int userId) async {
    ...
    ResponseModel _response = await chatServiceInterFace.sendMessage(
      message,
      userId,
  ...
  void pickMultipleMedia(
    ...
    if (isRemove) {
      ...
      final XFile? pickedImage =
          await ImageValidationHelper.validateAndPickImage(
            source: ImageSource.gallery,
            context: Get.context!,
          );

      if (pickedImage != null) {
        pickedMediaFileModelList?.add(
          MediaFileModel(
            file: pickedImage,
      ...
      for (PlatformFile file in validatedFiles) {
        if (isVideoExtension(file.path ?? '')) {
          ...
          double size = await ImageValidationHelper.getImageSizeFromXFile(
            file.xFile,
          );
          ...
          if (thumbnailPath != null &&
              size <=
                  (configModel?.systemGeneralFileUploadMaxSize ??
            ...
            showCustomSnackBarWidget(
              '${'maximum_file_size'.tr} ${(configModel?.systemImageFileUploadMaxSize ?? AppConstants.fileImageMaxLimit)}MB',
            );
          }
        } else {
          double value = await ImageValidationHelper.getImageSizeFromXFile(
            file.xFile,
          );

          ConfigModel? configModel = Get.find<SplashController>().configModel;
          if (value >
              (configModel?.systemImageFileUploadMaxSize ??
                  AppConstants.fileImageMaxLimit)) {
            showCustomSnackBarWidget(
              '${'maximum_file_size'.tr} ${(configModel?.systemImageFileUploadMaxSize ?? AppConstants.fileImageMaxLimit)}MB',
            );
          } else {
    ...
    pickedMediaFileModelList?.forEach((element) {
      if (ImageValidationHelper.getFileSizeFromXFileSync(element.file!) >
          (getExtractSizeInMB(
  ...
  Future<bool> _isCrossMediaMaxLimit() async =>
      ...
      await ImageValidationHelper.getMultipleImageSizeFromXFile(
            getXFileFromMediaFileModel(pickedMediaFileModelList ?? []) ?? [],
          ) >
    ...
    if (isRemove) {
      ...
      if (platformFile != null) {
        ...
        for (var file in platformFile) {
          double fileSize =
              ImageValidationHelper.getFileSizeFromPlatformFileToDouble(file);
          ...
          if (!hasInvalidFile &&
              fileSize >
                  (Get.find<SplashController>()
                      ...
                      AppConstants.maxSizeOfASingleFile)) {
            showCustomSnackBarWidget(
              '${'maximum_file_size'.tr} ${(Get.find<SplashController>().configModel?.systemGeneralFileUploadMaxSize ?? AppConstants.maxSizeOfASingleFile)}MB',
            );
        ...
        validatedFiles.forEach((element) {
          if (ImageValidationHelper.getFileSizeFromPlatformFileToDouble(
                element,
              ) >
              AppConstants.maxSizeOfASingleFile) {
            _singleFIleCrossMaxLimit = true;
        ...
        if (_pickedFiles?.length == AppConstants.maxLimitOfTotalFileSent &&
            validatedFiles.length > AppConstants.maxLimitOfTotalFileSent) {
          _pickedFIleCrossMaxLength = true;
        }
        if (_pickedFiles?.length == AppConstants.maxLimitOfTotalFileSent &&
            ImageValidationHelper.getMultipleFileSizeFromPlatformFiles(
                  validatedFiles,
                ) >
                AppConstants.maxLimitOfFileSentINConversation) {
  ...
  String getChatTimeWithPrevious(Message currentChat, Message? previousChat) {
    ...
    if (previousChat?.createdAt == null) {
      ...
      previousConversationDateTime = DateConverter.isoUtcStringToLocalTimeOnly(
  ...
  String getChatTime(String todayChatTimeInUtc, String? nextChatTimeInUtc) {
    ...
    if (nextChatTimeInUtc == null) {
      String chatTime = DateConverter.isoStringToLocalDateAndTime(
        todayChatTimeInUtc,
      );
      return chatTime;
    } else {
      nextConversationDateTime = DateConverter.isoUtcStringToLocalTimeOnly(
        nextChatTimeInUtc,
      );
      if (todayConversationDateTime.difference(nextConversationDateTime) <
        ...
        if ((currentDate.weekday - 1 == 0 ? 7 : currentDate.weekday - 1) ==
          ...
          chatTime = DateConverter.convertStringTimeToDate(
            todayConversationDateTime,
          ).toString();
        }
      } else if (currentDate.weekday == todayConversationDateTime.weekday &&
  ...
  String getConversionTime(String? conversationTime) {
    if (conversationTime!.isNotEmpty) {
      DateTime currentDate = DateTime.now();
      DateTime todayConversationDateTime =
          DateConverter.isoUtcStringToLocalTimeOnly(conversationTime);
