...
class _MediaViewerScreenState extends State<MediaViewerScreen> {
    ...
    if (_fromNetwork && (widget.serverMedia?.isNotEmpty ?? false)) {
      ...
      if (chatController.isVideoExtension(
        ...
        controller = VideoPlayerController.file(
          File(widget.localMedia![currentIndex].path),
        );
      }
    }
        ...
        body: Stack(
          children: [
            PageView.builder(
                ...
                children: [
                  ...
                  _fromNetwork
                      ? chatController.isVideoExtension(
                            ...
                            ? Flexible(
                                child: Center(
                                  child: Chewie(controller: chewController!),
                                ),
                              )
                            : Expanded(
              ...
              itemCount: _fromNetwork
                  ? widget.serverMedia?.length
                  : widget.localMedia?.length,
            ...
            if (_isButtonAvailable()) ...[
              ...
              Positioned.fill(
                child: Opacity(
                  opacity: currentIndex == (isLtr ? 0 : _getLastIndex())
                      ? 0.3
                      : 1,
                  child: Align(
              ...
              Positioned.fill(
                child: Opacity(
                  opacity: currentIndex == (isLtr ? _getLastIndex() : 0)
                      ? 0.3
                      : 1,
                  child: Align(
  ...
  Future<void> _download(String? url) async {
    ...
    if (status.isGranted) {
      ...
      Get.find<ChatController>().downloadFile(
        url,
        directory!.path,
        "${directory.path}/$url",
        url,
  ...
  bool _isButtonAvailable() => _fromNetwork
      ? (widget.serverMedia?.length ?? 0) > 1
      : (widget.localMedia?.length ?? 0) > 1;
