...
class NotificationController extends GetxController implements GetxService {
  ...
  Future<void> getNotificationList(int offset, {bool reload = true}) async {
    ...
    _isLoading = true;
    Response response = await notificationServiceInterface.getNotificationList(
      offset,
    );
    if (response.statusCode == 200) {
      if (offset == 1) {
        ...
        _notificationList!.addAll(
          NotificationModel.fromJson(response.body).notifications!,
        );
      } else {
        _notificationModel!.totalSize = NotificationModel.fromJson(
          response.body,
        ).totalSize;
        _notificationModel!.offset = NotificationModel.fromJson(
          response.body,
        ).offset;
        _notificationModel!.notifications!.addAll(
          NotificationModel.fromJson(response.body).notifications!,
        );
      }
    } else {
