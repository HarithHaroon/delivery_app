...
import 'package:rawabit_delivery_app/features/live_tracking/widgets/expendale_bottom_sheet_widget.dart';
...
class OrderLiveTrackingScreen extends StatefulWidget {
  ...
  @override
  State<OrderLiveTrackingScreen> createState() =>
      _OrderLiveTrackingScreenState();
}
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.orderModel!.shippingAddress != null) {
      Get.find<OrderController>().setSelectedOrderLatLng(
        LatLng(
          (widget.orderModel!.shippingAddress!.latitude != null &&
                  widget.orderModel!.shippingAddress!.latitude != '0' &&
                  widget.orderModel!.shippingAddress!.latitude != '')
              ? double.parse(widget.orderModel!.shippingAddress!.latitude!)
              : _defaut.latitude,

          (widget.orderModel!.shippingAddress!.longitude != null &&
                  widget.orderModel!.shippingAddress!.longitude != '0' &&
        ...
        builder: (riderController) {
          return ExpandableBottomSheet(
                  ...
                  builder: (context, markers) {
                    return GoogleMap(
                      ...
                      onMapCreated: (GoogleMapController controller) {
                        ...
                        LatLng destination =
                            widget.orderModel!.shippingAddress != null
                            ? LatLng(
                                double.parse(
                                  widget.orderModel!.shippingAddress!.latitude!,
                                ),
                        ...
                        riderController.getPolyline(
                          from: riderController.initialPosition,
                          to: destination,
                        );
                        riderController.setFromToMarker(
                          from: riderController.initialPosition,
                          to: destination,
                        );
                      },
                      ...
                      polylines: Set<Polyline>.of(
                        riderController.polylines.values,
                      ),
            ...
            persistentHeader: Container(
              ...
              child: Text(
                '${riderController.distance != null ? riderController.distance!.toStringAsFixed(2) : 0} km',
