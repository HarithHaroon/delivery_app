...
import 'package:rawabit_delivery_app/features/live_tracking/domain/models/distance_model.dart';
...
class RiderController extends GetxController implements GetxService {
  ...
  PolylinePoints polylinePoints = PolylinePoints(
    apiKey: AppConstants.polylineMapKey,
  );
  ...
  Future<double?> getDistanceInKM(
    ...
    _distance = -1;
    Response response = await riderRepo.getDistanceInMeter(
      originLatLng,
      destinationLatLng,
    );
    try {
  ...
  Future<void> getPolyline({LatLng? from, LatLng? to}) async {
    ...
    try {
      ...
      if (response.routes.isEmpty) {
        debugPrint(
          'getPolyline: no routes found. error=${response.errorMessage}',
        );
      ...
      if (routePoints != null && routePoints.isNotEmpty) {
        ...
        final PolylineResult legacy = polylinePoints.convertToLegacyResult(
          response,
        );
        if (legacy.points.isNotEmpty) {
          polylineCoordinates.addAll(
            legacy.points.map((p) => LatLng(p.latitude, p.longitude)),
          );
        } else {
          debugPrint(
            'getPolyline: no polyline points available after fallbacks.',
          );
        }
      }
    } catch (e, st) {
  ...
  void setFromToMarker({required LatLng from, required LatLng to}) async {
    ...
    try {
      ...
      mapController!.moveCamera(
        CameraUpdate.newCameraPosition(
          CameraPosition(bearing: bearing, target: centerBounds, zoom: 16),
        ),
      );
