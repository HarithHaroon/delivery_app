...
import 'package:rawabit_delivery_app/features/maintenance/maintenance_screen.dart';
import 'package:rawabit_delivery_app/features/notification/domain/models/notification_body.dart';
import 'package:rawabit_delivery_app/features/notification/screens/notification_screen.dart';
import 'package:rawabit_delivery_app/features/order/domain/models/order_model.dart';
import 'package:rawabit_delivery_app/features/order_details/screens/order_details_screen.dart';
...
import 'package:rawabit_delivery_app/features/splash/domain/models/config_model.dart'
    hide Colors;
import 'package:rawabit_delivery_app/features/update/screen/update_screen.dart';
import 'package:rawabit_delivery_app/features/wallet/screens/wallet_screen.dart';
import 'package:rawabit_delivery_app/helper/network_info.dart';
...
import 'package:rawabit_delivery_app/features/onboard/screens/onboarding_screen.dart';
...
class SplashScreenState extends State<SplashScreen> {
  ...
  void initState() {
    ...
    FirebaseMessaging.instance.subscribeToTopic(
      AppConstants.maintenanceModeTopic,
    );
  ...
  void _route() {
    ...
    Get.find<SplashController>().getConfigData().then((isSuccess) async {
      ...
      if (isSuccess) {
        ...
        DeliveryManAppVersionControl? appVersion =
            config?.deliveryManAppVersionControl;
          ...
          if (compareVersions(minimumVersion!, AppConstants.appVersion) == 1) {
            ...
            if (widget.body != null) {
              ...
              switch (notificationType.toLowerCase()) {
                case 'chatting':
                  {
                    Get.offAll(
                      DashboardScreen(
                        pageIndex: 2,
                        chatIndex:
                            widget.body?.messageKey == 'message_from_customer'
                            ? 1
                            : widget.body?.messageKey == 'message_from_seller'
                            ? 0
                            : 3,
                      ),
                    );
                ...
                case 'order':
                  {
                    Get.offAll(
                      OrderDetailsScreen(
                        orderModel: OrderModel(id: widget.body?.orderId),
                ...
                case 'wallet':
                  {
                    if (widget.body?.type == 'wallet' &&
                        widget.body?.messageKey ==
  ...
  int compareVersions(String version1, String version2) {
    ...
    for (int i = 0; i < maxLength; i++) {
      int v1Part = i < v1Components.length
          ? int.tryParse(v1Components[i]) ?? 0
          : 0;
      int v2Part = i < v2Components.length
          ? int.tryParse(v2Components[i]) ?? 0
          : 0;
