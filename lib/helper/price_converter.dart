...
class PriceConverter {
  static String convertPrice(
    ...
    bool singleCurrency =
        Get.find<SplashController>().configModel!.currencyModel ==
        'single_currency';
    bool inRight =
        Get.find<SplashController>().configModel!.currencySymbolPosition ==
        'right';
    ...
    try {
      final decimal =
          Get.find<SplashController>().configModel!.decimalPointSettings ?? 1;

      double finalPrice = singleCurrency
          ? price!
          : price! *
                Get.find<SplashController>().myCurrency!.exchangeRate! *
      ...
      return '${inRight ? '' : Get.find<SplashController>().myCurrency!.symbol}'
          '${(singleCurrency ? price : price! * Get.find<SplashController>().myCurrency!.exchangeRate! * (1 / Get.find<SplashController>().usdCurrency!.exchangeRate!))!.toStringAsFixed(Get.find<SplashController>().configModel!.decimalPointSettings ?? 1).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}'
          '${inRight ? Get.find<SplashController>().myCurrency!.symbol : ''}';
    } catch (e) {
