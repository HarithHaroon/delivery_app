...
class ShopHelper {
  ...
  static bool _checkInHouseVacation(
    BuildContext context,
    ...
    try {
      ...
      if (configModel?.inHouseVacationAdd?.vacationDurationType ==
          VacationDurationType.untilChange) {
        return true;
      }

      if (configModel?.inHouseVacationAdd?.vacationStartDate == null ||
          configModel?.inHouseVacationAdd?.vacationEndDate == null) {
        return false;
      }

      return currentDate.isAfter(
            configModel!.inHouseVacationAdd!.vacationStartDate!,
          ) &&
          currentDate.isBefore(
            configModel.inHouseVacationAdd!.vacationEndDate!,
          );
    } catch (e) {
  ...
  static bool _checkSellerVacation({
    required DateTime? startDate,
    required DateTime? endDate,
